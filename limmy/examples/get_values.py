import limmy
from limmy.VESC.messages import GetValues, SetRPM, SetCurrent, SetRotorPositionMode, GetRotorPosition
import serial
import time
...
def get_values_example():
    with serial.Serial(serialport, baudrate=115200, timeout=0.05) as ser:
        try:
            ...
            ser.write(limmy.encode(SetRotorPositionMode(SetRotorPositionMode.DISP_POS_MODE_ENCODER)))
            while True:
                ...
                ser.write(limmy.encode(SetRPM(10000)))

                # Request the current measurement from the vesc
                ser.write(limmy.encode_request(GetValues))

                # Check if there is enough data back for a measurement
                if ser.in_waiting > 61:
                    (response, consumed) = limmy.decode(ser.read(61))
            ...
            # Turn Off the VESC
            ser.write(limmy.encode(SetCurrent(0)))
